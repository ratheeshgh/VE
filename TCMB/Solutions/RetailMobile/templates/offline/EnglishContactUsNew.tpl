<div class="ContactContainer">
	<p class="Size12 Medium titleContact">LLAMANOS AL CENTRO DE ATENCION AL CLIENTE</p>
	<div class="contactCallRow">
		<a href="tel:5078001300" class="col-xs-12 contactCallBtn">
			<div class="col-xs-3 iconContact ContactCall"></div>
			<div class="col-xs-9 infoContact">
				<p class="from Size12">DESDE PANAMA</p>
				<p class="Size12">(507)800-1300</p>
			</div>
		</a>
	</div>
	<div class="contactCallRow">
		<a href="tel:05002263726" class="col-xs-12 contactCallBtn">
			<div class="col-xs-3 iconContact ContactCall"></div>
			<div class="col-xs-9 infoContact">
				<p class="from Size12">DESDE VENEZUELA</p>
				<p class="Size12">0500BANESCO (0500-2263726)</p>
			</div>
		</a>
	</div>
	<div class="contactCallRow">
		<a href="tel:0115072822013" class="col-xs-12 contactCallBtn">
			<div class="col-xs-3 iconContact ContactCall"></div>
			<div class="col-xs-9 infoContact">
				<p class="from Size12">DESDE ESTADOS UNIDOS</p>
				<p class="Size12">011-507-282-2013</p>
			</div>
		</a>
	</div>
    <p class="Size12 Medium titleContact titleContactMail">ENVIAMOS UN CORREO ELECTRONICO SEGURO A</p>
	 <div class="contactCallRow">
		<a href="mailto:ACLIENTE_PA@BANESCO.COM" class="col-xs-12 contactCallBtn">
			<div class="col-xs-3 iconContact ContactEmail"></div>
			<div class="col-xs-9 infoContact">
				<p class="from Size12">DESDE ESTADOS UNIDOS</p>
				<p class="Size12">ACLIENTE_PA@BANESCO.COM</p>
			</div>
		</a>
	</div> 
</div>
