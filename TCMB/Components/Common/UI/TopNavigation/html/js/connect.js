...
function buttonClicked(val, p_valMand, p_rowId, ns, scrollToButton, id, saveFormData, validateAllFields, buttonConfirmMsg, inlineErrors, valMand, idsToValidate)
{
 ...
 if (window.getFormElems && saveFormData)
 {
	if (idsToValidate){
 	...
 	var rowSelectors = new Set();
 	var urlProps = "";
 	for (var i = 0; i < elements.length; i++) {
 		var el = elements[i];
		var name = el.name;
		if (el.id.indexOf(SELECTOR_INDICATOR) > 0) {
			rowSelectors.add(name);
		}
	}
	if (rowSelectors.size > 0) {
		for (rowSelector in rowSelectors) {
			urlProps = urlProps + rowSelector + ";";
		}
